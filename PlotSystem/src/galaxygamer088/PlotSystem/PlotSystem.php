...
const CROSSING = 4; //blue
    ...
    public function move(PlayerMoveEvent $ev){
        ...
        if($this->isPlotWorld($p->getWorld())){
            $plotTo = $this->getPlotIdByPosition(round($ev->getTo()->getX(), 3), round($ev->getTo()->getZ(), 3));
            $plotFrom = $this->getPlotIdByPosition(round($ev->getFrom()->getX(), 3), round($ev->getFrom()->getZ(), 3));
    ...
    public function getAllPlayer() : array{
        $playerList = [];
        $count = 1;
        if($this->permissions->getNested("Options.ConnectToSupporterPlugin") == "true"){
